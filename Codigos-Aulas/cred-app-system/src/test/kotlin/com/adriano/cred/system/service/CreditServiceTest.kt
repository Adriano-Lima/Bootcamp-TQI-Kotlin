...
import com.adriano.cred.system.service.impl.CreditService
...
class CreditServiceTest {
  ...
  fun `should return credit for a valid customer and credit code`() {
    ...
    //when
    val actual: Credit = creditService.findByCredCode(customerId, creditCode)
    //then
    Assertions.assertThat(actual).isNotNull
    Assertions.assertThat(actual).isSameAs(credit)
  ...
  fun `should throw BusinessException for invalid credit code`() {
    ...
    //when
    //then
    Assertions.assertThatThrownBy { creditService.findByCredCode(customerId, invalidCreditCode) }
      .isInstanceOf(BusinessException::class.java)
  ...
  fun `should throw IllegalArgumentException for different customer ID`() {
    ...
    //when
    //then
    Assertions.assertThatThrownBy { creditService.findByCredCode(customerId, creditCode) }
  ...
  companion object {
    private fun buildCredit(
      ...
      numberOffInstallments = numberOfInstallments,
      customer = customer
    )
  }
}